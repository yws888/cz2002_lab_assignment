...
// Get details of course - ID + Name + ProfName + LectOnly(Y/N) + WithTutorial(Y/N) + WithLab(Y/N)
// Error handling to ensure information is not already in database
// Print out updated course list once finalized details
	...
	public void getCourseDetails() {
		...
		while (true) {
			...
			// else continue back to asking for correct input
				...
				System.out.println("Wrong faculty Name");
		}
		...
		if (checkCourseName(newCourseName)) {
			...
			courseNameList.add(newCourseName);
		...
		while (true) {
			...
			// else continue back to asking for correct input
				...
				System.out.println("Enter correct professor.");
		}
		...
		while (true) {
			...
			if (lectureInput == 'Y' || lectureInput == 'y') {
				...
				// To return the course code, name vacancy as well as the professor assigned to
				// it
				// for that specific course under the faculty it is assgined to
				// ++ passCourseDetails(facultyName, courseCode, courseName, courseVacancy,
				// ProfName);
				break;
			}
